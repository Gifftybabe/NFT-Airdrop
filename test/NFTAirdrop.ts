...
describe("MerkleAirdropCoreFuncsTest", function () {
  ...
  it("Should allow owner to deposit token", async function () {
    await token.connect(owner).approve(merkleAirdrop, 500);
    await expect(merkleAirdrop.contractDeposit(500))
      .to.emit(merkleAirdrop, "contractDepositSuccessfully")
      .withArgs(owner.address, 500);
  });

  it("Should permit qualified users with BAYC to claim Token", async function () {
    ...
    ).to.emit(merkleAirdrop, "TokenClaimedSuccessfully");
  });

  it("Should dissallow a user without the NFT to claim tokens", async function () {
  ...
  it("Should not allow a user to claim tokens more than once", async function () {
    ...
    ).to.be.revertedWithCustomError(merkleAirdrop, "TokenAlreadyClaimed");
  });

  it("Should dissallow user to provides an invalid proof", async function () {
    ...
    ).to.be.revertedWithCustomError(merkleAirdrop, "YouAreNotQualified");
  });
});